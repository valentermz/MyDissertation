...
\begin{align} 
 ...
 \sum_{p\in\operatorname{Sing }\F_v} \hspace{-8pt}\BBindex{\F_v}{p} &= 16, \label{eq:BB}\\
...
\begin{theorem}[\cite{NoIndexTheorems}]\label{thm:hiddenRelation}
 There exist symmetric polynomials $P_0,P_1,P_2\in S$, homogeneous (with respect to the grading of $S$) of degree 14, such that every generic quadratic vector field satisfies the following equation
  ...
  \end{equation}
 Moreover, the above relation is independent from the identities (\ref{eq:EJ1})--(\ref{eq:CS}).
\end{theorem}
...
If $v=P\frac{\partial}{\partial x}+Q\frac{\partial}{\partial y}$ is a polynomial vector field of degree $n\geq2$ having $n^2$ non-degenerate singularities $p_1,\ldots,p_{n^2}\in\C^2$ then
\begin{align*}
\sum_{k=1}^{n^2}\frac{1}{\det{Dv(p_k)}}=0, \\
\sum_{k=1}^{n^2}\frac{\tr{Dv(p_k)}}{\det{Dv(p_k)}}=0.
\end{align*}
\end{corollary}
...
\begin{corollary}
  ...
  \[ \sum_{k=1}^4\frac{t_k^2}{d_k} + \sum_{j=1}^3 \frac{(\lambda_j+1)^2}{\lambda_j} = 16 . \]
\end{corollary}
...
% Cite Moldavskis and Lins-Neto results...
